defmodule EvilCrc32cTest do
  use ExUnit.Case

  doctest EvilCrc32c

  describe "calc/1" do
    test "calculates crc32c" do
      {:ok, source} =
        Base.decode64(
          ""
        )

      assert {:ok, 1_580_765_482} = EvilCrc32c.calc(source, false)
      assert {:ok, <<42, 145, 56, 94>>} = EvilCrc32c.calc(source)
    end

    test "fails to calculate crc32c for non-binary data" do
      assert {:error, :not_binary_data} = EvilCrc32c.calc(%{})
    end
  end

  describe "calc!/1" do
    test "calculates crc32c" do
      {:ok, source} =
        Base.decode64(
          ""
        )

      assert 1_580_765_482 = EvilCrc32c.calc!(source, false)
      assert <<42, 145, 56, 94>> = EvilCrc32c.calc!(source)
    end

    test "fails to calculate crc32c for non-binary data" do
      assert_raise ArgumentError, "argument error", fn ->
        EvilCrc32c.calc!(%{})
      end
    end
  end
end
